...
```mermaid
flowchart TD
    A[Start: Receive Request\n(user_id, upload_dir)] --> B[Extract Audio Paths\n(audio_paths list)]
    B --> C[Fetch User Data]

    C -->|Java API| C1[GET /fetch-user\n(user_id)]
    C -->|Fallback DB| C2[SELECT subscription_plan,\ncredits_available\nFROM user_credentials+user_credits]

    C --> D{Subscription Active?}
    D -->|No| D1[Java API POST /notify-subscription\nRaise Exception → HALT]
    D -->|Yes| E[Get Billing Config\n(cost_per_minute, max_files)]

    E --> F[Calculate Costs Per File]
    F --> F1[For each audio_file:\nGet Duration (ffprobe/librosa)]
    F1 --> F2[Cost = (duration / 60) * rate_per_minute]
    F2 --> F3[Sum total_cost = Σ file costs]

    F3 --> G{Plan Limit Exceeded?}
    G -->|Yes| G1[Raise Exception\nPlan Limit Exceeded → HALT]
    G -->|No| H{Credits >= total_cost?}

    H -->|Yes| H1[Set process_files = all audio_paths]
    H -->|No| I[Calculate Affordable Subset]

    I --> I1[Query DB: unprocessed files\n(recording_flag = False)]
    I1 --> I2[Cumulative add until cum_cost <= credits]
    I2 --> J{max_files == 0?}

    J -->|Yes| J1[Java API POST /notify-recharge\n→ HALT]
    J -->|No| K[Java API POST /ask-partial]

    K -->|No| K1[Java API POST /notify-recharge\n→ HALT]
    ...
    L7 --> L8[Mark File Done\n(recording_flag=True)]
    L8 --> L9[Move File to Completed Dir]

    L9 --> M[After All Done:\nCalc actual_cost = Σ processed file costs]
    ...
    N3 --> O[End: Return Success\n(status: ok, processed_files count)]
    D1 --> O
    G1 --> O
    J1 --> O
    K1 --> O
